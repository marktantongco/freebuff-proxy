...
(() => {
  ...
  // Reverse patterns for interpolated strings the zh patch renders via
  // regexes (most frequent families). Order matters: most specific first.
  const patterns = [
    ...
    [/^再坚持 (\d+) 天，即可每天额外获得 1 个会话$/, '$1 more days to unlock +1 bonus session every day'],
    [/^(\d+) 条排队中的提示$/, '$1 queued prompts'],
    [/^(\d+) 条排队中的提示?$/, '$1 queued prompt'],
    [/^(\d+) 个排队项目$/, '$1 queued items'],
    [/^(\d+) 像素$/, '$1 pixels'],
    ...
    [/^智能体修改了 (\d+) 个文件$/, 'Agent changed $1 files'],
  ]
  ...
  // Region display names zh → en (Intl.DisplayNames, reversed from zh patch).
  ...
  function applyContextRules(root) {
    ...
    for (const heading of contextCandidates(scope, '.turn-changes-head')) {
      ...
      const label = labels.find((candidate) => /^智能体修改了\s+\d+\s+个文件$/.test(candidate.textContent.trim()))
