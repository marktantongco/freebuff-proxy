...
var keyCatalog = []KeyDef{
	...
	{Key: "UNFIT_EGRESS", Group: GroupUpstream, Kind: "text", RestartOnly: true, Hidden: true,
		Default:     "direct",
		Description: `Egress identity recorded in the (egress, model) unfit registry; "direct" by default. When proxy egresses are re-introduced, give each egress its own label so a limited_ip refusal on one path never marks models unfit on another.`},
